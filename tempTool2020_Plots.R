...
umatillaBins_1third <- hyporheicBins(18, 2, 60, 182*86400, 0.25, 8.33, b=-1.39)

load("model_output/newUmatilla.RData")
load("model_output/newUmatilla13.RData")
load("model_output/newUmatilla23.RData")
...
hypolit <- newUmatilla13$cTemp$svValue
hypomed <- newUmatilla23$cTemp$svValue
hypohigh <- newUmatilla$cTemp$svValue
...
plot.zoo(newUmatilla13$cTemp$svValue[yr1.5], col = lowcol)
lines(as.zoo(newUmatilla23$cTemp$svValue[yr1.5]), col = medcol)
lines(as.zoo(newUmatilla$cTemp$svValue[yr1.5]), col = highcol)
...
for(i in 1:4){
  ...
  ranges[2,i] <- rangecalc(x = c(control[plotdays[i]],
                                 newUmatilla13$cTemp$svValue[plotdays[i]],
                                 newUmatilla23$cTemp$svValue[plotdays[i]],
                                 newUmatilla$cTemp$svValue[plotdays[i]]))
}
...
png("C:/Users/t24x137/Desktop/TempTool_2020/plots/DailyTemperature.png",
